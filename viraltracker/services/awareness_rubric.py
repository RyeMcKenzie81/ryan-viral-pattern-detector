...
- before/after: if a prominent branded product/bottle appears anywhere in the frame, product_aware (the transformation IS product-level proof for the visible product, satisfying the decisive rule's product-level-work requirement). With NO product shown (no brand, bottle, or product name anywhere): the viewer has nothing product-specific to presume — a visual transformation alone is a desired-outcome lead = solution_aware, and a mechanism / explanatory lead = solution_aware. (A transformation cannot be product_aware when no product is identifiable.)
- pure offer (discount / %-off / urgency / "back in stock" as the LEAD) = most_aware. OFFER PROMINENCE: an explicit promo trigger (SALE / %-off / promo code / BOGO / a named sale event like "New Year's Sale") rendered at BANNER SCALE — a major element of the visual hierarchy (a full-width band or among the largest type in the frame) — makes the ad most_aware EVEN WHEN testimonial / product-proof elements are also present: an offer only does work on settled desire, so the proof is in service of conversion. A SMALL promo badge/sticker subordinate to a dominant problem/solution/product message does NOT raise the stage — judge the dominant message. A hard CTA or a results-timeline selling merit is still NOT an offer.
