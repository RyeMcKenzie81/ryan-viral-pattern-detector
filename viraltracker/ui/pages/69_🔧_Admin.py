...
with tab_users:
    if org_id == "all":
        st.info("Select a specific organization to manage users.")
    else:
        org_service = _get_org_service()
        st.subheader("Organization Members")

        members = org_service.get_org_members(org_id)
        if members:
            import pandas as pd
            member_rows = []
            for m in members:
            ...
            with col_role:
                # Can't change own role, can't change owner if not superuser
                can_change = True
                ...
                if st.button("Update Role", disabled=not can_change, key="admin_update_role"):
                    if new_role != selected_member["role"]:
                        try:
                            org_service.update_member_role(org_id, selected_member["user_id"], new_role)
            ...
            with col_remove:
                ...
                if st.button(
                    ...
                    try:
                        org_service.remove_member(org_id, selected_member["user_id"])
        ...
        with st.form("add_member_form"):
            ...
            if add_submitted and new_user_id:
                try:
                    org_service.add_member(org_id, new_user_id, new_role)
...
with tab_features:
    if org_id == "all":
        st.info("Select a specific organization to manage features.")
    else:
        ...
        current_features = feature_service.get_org_features(org_id)
        ...
        if changes:
            if st.button("Save Feature Changes", key="admin_save_features"):
                for fk, enabled in changes.items():
                    feature_service.set_feature(org_id, fk, enabled)
...
with tab_limits:
    if org_id == "all":
        st.info("Select a specific organization to manage usage limits.")
    else:
        limit_service = _get_limit_service()
        st.subheader("Usage Limits")

        # All limit types with display info
        limit_types = [
            {
                "type": LimitType.MONTHLY_COST,
                "label": "Monthly Cost (USD)",
                "period": "monthly",
                "help": "Maximum monthly spend in USD",
                "format": "$",
            },
            {
                "type": LimitType.MONTHLY_TOKENS,
                "label": "Monthly Tokens",
                "period": "monthly",
                "help": "Maximum total tokens per month",
                "format": "tokens",
            },
            {
                "type": LimitType.DAILY_REQUESTS,
                "label": "Daily API Requests",
                "period": "daily",
                "help": "Maximum API requests per day",
                "format": "requests",
            },
            {
                "type": LimitType.DAILY_ADS,
                "label": "Daily Ad Generations",
                "period": "daily",
                "help": "Maximum ad generations per day",
                "format": "ads",
            },
        ]

        # Get current limits and usage
        current_limits = limit_service.get_limits(org_id)
        ...
        for lt_info in limit_types:
            ...
            with col_enabled:
                ...
                new_enabled = st.checkbox(
                    ...
                    key=f"admin_limit_enabled_{lt}",
                )

            # Show current usage
            try:
                status = limit_service.get_current_period_usage(org_id, lt)
                ...
                if st.button("Save", key=f"admin_limit_save_{lt}"):
                    if new_val > 0:
                        limit_service.set_limit(
                            org_id, lt, float(new_val), lt_info["period"], new_thresh, new_enabled
                        )
            ...
            with col_delete:
                if existing:
                    if st.button("Remove", key=f"admin_limit_del_{lt}"):
                        limit_service.delete_limit(org_id, lt)
