#!/usr/bin/env python3
"""
Test lowering green threshold from 0.55 to 0.52
Export all tweets >= 0.52 for quality review
"""
import csv
...
def main():
    print("🔍 Testing threshold 0.52 (vs current 0.55)...")
    ...
    for tweet, embedding in zip(tweets, embeddings):
        ...
        if result.total_score >= 0.52:
            threshold_052.append({
                'tweet': tweet,
                'scores': result
            })
    ...
    # Sort by score
    threshold_052.sort(key=lambda x: x['scores'].total_score, reverse=True)

    print(f"\n📊 Results:")
    print(f"  Current greens (≥ 0.55): {len(current_greens)}")
    print(f"  With 0.52 threshold: {len(threshold_052)}")
    print(f"  Additional tweets: {len(threshold_052) - len(current_greens)}")

    # Export to CSV
    output_file = '/Users/ryemckenzie/Downloads/tweets_threshold_052.csv'
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as f:
        ...
        for item in threshold_052:
            tweet = item['tweet']
            scores = item['scores']

            # Label under current threshold (0.55)
            current_label = 'green' if scores.total_score >= 0.55 else 'yellow'
            # Label under new threshold (0.52)
            new_label = 'green'  # All are >= 0.52
    ...
    bands = {
        '0.55-0.70': [x for x in threshold_052 if 0.55 <= x['scores'].total_score < 0.70],
        '0.52-0.55': [x for x in threshold_052 if 0.52 <= x['scores'].total_score < 0.55],
    }
    ...
    # Show NEW tweets that would become green (0.52-0.55 range)
    new_greens = [x for x in threshold_052 if 0.52 <= x['scores'].total_score < 0.55]

    print("\n" + "="*80)
    print(f"NEW 'GREEN' TWEETS (0.52-0.55 range) - {len(new_greens)} tweets")
    print("="*80)

    for i, item in enumerate(new_greens[:10], 1):  # Show first 10
        tweet = item['tweet']
        scores = item['scores']
        ...
        print(f"Engagement: {tweet.views:,} views, {tweet.likes} likes")
        print(f"Components: vel={scores.velocity:.3f}, rel={scores.relevance:.3f}, open={scores.openness:.3f}")

    if len(new_greens) > 10:
        print(f"\n... and {len(new_greens) - 10} more in the CSV")
