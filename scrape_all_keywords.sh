...
# Scrape All Keywords (Last 48 Hours)
# Re-scrape all 19 keywords with fresh data using 0.50 threshold

# Create output directory
mkdir -p ~/Downloads/keyword_analysis_all
...
echo "========================================"
echo "Scraping All Keywords (48 Hours)"
echo "========================================"
...
for term in "${terms[@]}"; do
  ...
  source venv/bin/activate && python -m viraltracker.cli.main twitter analyze-search-term \
    ...
    --min-likes 0 \
    --skip-comments \
    --report-file ~/Downloads/keyword_analysis_all/${filename}_report.json
...
echo "========================================"
echo "Results saved to: ~/Downloads/keyword_analysis_all/"
...
for term in "${terms[@]}"; do
  filename=$(echo "$term" | tr ' ' '_')
  report_file=~/Downloads/keyword_analysis_all/${filename}_report.json
...
echo "========================================"
echo ""
echo "Step 2: Generate comments for all greens from last 48 hours"
echo "========================================"
...
source venv/bin/activate && python -m viraltracker.cli.main twitter export-comments \
  --project yakety-pack-instagram \
  --out ~/Downloads/keyword_greens_48h.csv \
  --status pending

echo ""
echo "✅ Complete! Greens exported to: ~/Downloads/keyword_greens_48h.csv"
