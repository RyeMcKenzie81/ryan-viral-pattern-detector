# Phase 8: Full Autonomous Intelligence — Implementation Plan

**Branch**: `feat/ad-creator-v2-phase0`
**Created**: 2026-02-16
**Status**: 8A COMPLETE — 8B PLANNING
...
**8A — Core Intelligence** (this plan, ships first):
1. **Few-Shot Exemplar Library** — Curate 20-30 calibration ads per brand, inject 3-5 most similar exemplars via embedding similarity into review prompts
2. **Adaptive Threshold Calibration** — Weekly cron job that auto-proposes new `quality_scoring_config` based on false positive/negative rates (operator approval in Settings before activation)
3. **Visual Embedding Space** — Gemini Flash extracts structured visual descriptors → OpenAI text-embedding-3-small → pgvector for exemplar similarity + "more like this" (Winner Evolution)
   - **Model note**: Parent plan (Section 14) specifies "Gemini 3 Flash" — not yet available. 8A uses `gemini-2.0-flash`; upgrade when 3.0 ships. The `extraction_model` column on `visual_embeddings` tracks which model was used, enabling re-extraction after upgrade.
4. **Interaction Effect Detection** — Detect top 15 element pair interactions from `creative_element_rewards` joined with `generated_ads.element_tags`, surface in Genome advisory + Settings read-only view
5. **FatigueScorer** — Hybrid predictive fatigue: template-level decay + element-combo modifier, replaces reactive fatigue detection in Phase 7A's anti-fatigue refresh

**8B — Advanced Intelligence** (separate plan, after 8A ships):
...
### 1.3 Clarifying Questions — Answered
...
| 3 | Calibration approval? | Settings page review. Weekly job proposes → operator Activates or Dismisses. Full audit trail. Safety rails: min sample size, bounded delta limits. |
...
| 6 | FatigueScorer input? | Hybrid: template-level base decay (days since template_id last used for product+audience) + element-combo modifier (hook_type × color_mode × template_category). Combo modifier neutral when data sparse. Score bounded [0.2, 1.0]. |
| 7 | Embedding method? | Text descriptors only for 8A. Gemini Flash extracts descriptors → embed with text-embedding-3-small. Schema fields for future hybrid upgrade. Defer image embeddings to 8B. |
...
| Adaptive Calibration | Service + Weekly Cron Job | Deterministic analysis → propose → human approve. No AI branching. |
...
**No pydantic-graph needed** — all features are either batch jobs, UI-driven services, or stateless scorers.
...
│                        Weekly Cron Jobs                         │
│                                                                 │
│  quality_calibration job ──► QualityCalibrationService          │
│       ├── Analyze override rates (false pos/neg)                │
│       ├── Propose new quality_scoring_config                    │
│       └── Store as inactive, pending operator approval          │
│                                                                 │
│  genome_validation job ──► InteractionDetectorService           │
│       ├── Compute pairwise element effect sizes                 │
│       ├── Store top 15 in element_interactions                  │
│       └── Update Genome advisory context                        │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│                    Visual Embedding Pipeline                     │
│                                                                 │
│  On ad generation complete:                                     │
│       ├── Gemini Flash extracts structured descriptors          │
│       ├── OpenAI text-embedding-3-small embeds descriptor text  │
│       └── Store in visual_embeddings (pgvector VECTOR(1536))    │
│                                                                 │
│  On review (exemplar injection):                                │
│       ├── Retrieve ad's visual embedding                        │
│       ├── pgvector similarity search → top 5 exemplars          │
│       └── Inject into review prompt as few-shot examples        │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│                    Template Scoring Pipeline                     │
│                                                                 │
│  Existing: AssetMatch, UnusedBonus, CategoryMatch,              │
│            AwarenessAlign, AudienceMatch, BeliefClarity,        │
│            Performance                                          │
│  NEW:      FatigueScorer (hybrid template + element combo)      │
│                                                                 │
│  Weight presets updated:                                         │
...
│                                                                 │
│  Results Dashboard:                                             │
│    + "Mark as Exemplar" button (gold_approve/gold_reject/edge)  │
│                                                                 │
│  Settings Page:                                                 │
│    + Calibration Proposals tab (view/activate/dismiss)          │
│    + Interaction Effects tab (read-only knowledge base)         │
│    + Exemplar Library tab (view/manage exemplars per brand)     │
└─────────────────────────────────────────────────────────────────┘
```
...
|--------|-------|---------|
| None needed | — | All new data goes in dedicated tables |
...
| 10 | UI: Exemplar management | UI | `ui/pages/21b_🎨_Ad_Creator_V2.py` | "Mark as Exemplar" button in results dashboard |
| 11 | UI: Settings enhancements | UI | `ui/pages/64_⚙️_Platform_Settings.py` | Calibration Proposals, Interaction Effects, Exemplar Library tabs |
...
CREATE TABLE IF NOT EXISTS visual_embeddings (
    ...
    extraction_model TEXT NOT NULL DEFAULT 'gemini-2.0-flash',  -- Parent plan (Section 14) says "Gemini 3 Flash" — not yet available; use gemini-2.0-flash for 8A, upgrade when 3.0 ships
...
class VisualDescriptorService:
    ...
    async def extract_and_store(
        ...
        """Full pipeline: extract → embed → store. Returns visual_embeddings.id."""
...
class QualityCalibrationService:
    ...
    async def propose_calibration(
        ...
        If any validation fails → log error + still persist a calibration_proposals row
...
class InteractionDetectorService:
    ...
    async def detect_interactions(
        ...
        (NOT creative_element_scores — we need per-ad reward + tag pairs, not
...
class FatigueScorer(TemplateScorer):
    ...
    Base decay: e^(-λ * days_since_template_last_used)
...
Tables created:
1. `exemplar_library` — curated calibration ads
2. `visual_embeddings` — structured descriptors + pgvector
3. `element_interactions` — pairwise element effects
4. `element_combo_usage` — combo usage tracking for fatigue
5. `calibration_proposals` — pending threshold changes
...
VisualDescriptorService:
- `extract_descriptors()` — Gemini Flash prompt returning structured JSON
- `embed_descriptors()` — OpenAI text-embedding-3-small
- `extract_and_store()` — full pipeline: extract → embed → insert
- `find_similar_ads()` — pgvector cosine similarity query
- `get_embedding()` — lookup by generated_ad_id

ExemplarService:
- `auto_seed_exemplars()` — query `ad_review_overrides`, classify by override type, enforce diversity + per-brand cap
- `mark_as_exemplar()` / `remove_exemplar()` — CRUD with cap enforcement
- `get_exemplars()` / `get_exemplar_stats()` — read queries
- `find_similar_exemplars()` — pgvector similarity restricted to exemplar_library
- `build_exemplar_context()` — format 3-5 exemplars (balanced categories) for review prompt
...
QualityCalibrationService:
- `analyze_overrides()` — compute false positive/negative rates from `ad_review_overrides` window
- `propose_calibration()` — generate new threshold proposal with safety rails
  ...
  - If insufficient evidence → status = 'insufficient_evidence'
- `activate_proposal()` — create new `quality_scoring_config` version, deactivate old
- `dismiss_proposal()` — mark dismissed with reason
- `get_pending_proposals()` / `get_proposal_history()` — read queries

InteractionDetectorService:
- `detect_interactions()` — pairwise analysis:
  ...
  2. For each pair of TRACKED_ELEMENTS, compute:
     ...
     - Expected reward = E[A] × E[B] (marginal independence)
...
- `get_top_interactions()` — read top-N from table
- `format_advisory_context()` — natural language summary for prompt
...
**Files**:
- `viraltracker/services/template_scoring_service.py` — add `FatigueScorer` class + update scorer lists + weight presets
- `viraltracker/pipelines/ad_creation_v2/nodes/compile_results.py` — track combo usage after generation
...
**Files**:
- `viraltracker/pipelines/ad_creation_v2/services/review_service.py` — modify `_build_rubric_prompt()` to accept optional `exemplar_context: str` parameter and inject into Stage 2 + Stage 3 prompts
- `viraltracker/pipelines/ad_creation_v2/services/review_service.py` — modify `_run_rubric_review_claude()` and `_run_rubric_review_gemini()` to pass exemplar context through to `_build_rubric_prompt()`
- `viraltracker/pipelines/ad_creation_v2/services/review_service.py` — modify `review_ad_staged()` to accept optional `exemplar_context: str` and pass to both Stage 2/3
- `viraltracker/pipelines/ad_creation_v2/nodes/review_ads.py` — wire exemplar lookup before `review_service.review_ad_staged()` call
...
### Example 1 [APPROVED — Gold Standard]
...
**Files**:
- `viraltracker/worker/scheduler_worker.py` — add `execute_quality_calibration_job()`
- `viraltracker/worker/scheduler_worker.py` — enhance `execute_genome_validation_job()` with interaction detection
- `viraltracker/services/creative_genome_service.py` — enhance `get_performance_context()` with interaction data
...
**Files**:
- `viraltracker/ui/pages/21b_🎨_Ad_Creator_V2.py` — "Mark as Exemplar" button in results dashboard section
- `viraltracker/ui/pages/64_⚙️_Platform_Settings.py` — Calibration Proposals, Interaction Effects, Exemplar Library tabs

Results Dashboard (`21b_🎨_Ad_Creator_V2.py`):
...
Settings Page (`64_⚙️_Platform_Settings.py`, new tabs):
...
**Files** (concrete names — no glob patterns):
...
Coverage:
- ExemplarService: auto-seed logic (override chain filtering — superseded_by IS NULL), cap enforcement, diversity constraints, similarity search, build_exemplar_context format
